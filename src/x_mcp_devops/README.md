...
- `register_tools.js` – **post-install fix script** that registers the DevOps tools with the MCP Core when installed.

`updateScriptInclude` checks the `instance_name` property to prevent updates on non‑dev instances.
